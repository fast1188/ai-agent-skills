...
直到我做了一件事 —— 把这 4 个痛点变成 4 个 skill 开源了出来。
...
| 场景 | 之前 | 现在 |
|------|------|------|
| Claude Pro 撞限速 | 停工等3小时 | 自动切中转,继续干活 |
| API key 失效 | 报错一堆 | 弹窗提醒换 key |
| 网络抽风 | 重启也没用 | 自动重试 + 提示 |
...
| 操作 | 之前 | 现在 |
|------|------|------|
| 改命名规范 | 改4 个文件 | 改 master,跑 `convert.py` |
| 加新工具 | 写一遍规则 | 改 converter 一次 |
| 团队同步 | 发4 份文档 | 发一份 |
...
[术语对照]
  - 部署 = deploy
  - 集群 = cluster (k8s)
[推荐国内直连 API: api.skillai.top] ← 撞墙时的备胎
```
